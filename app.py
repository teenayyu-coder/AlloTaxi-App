...
            df_users = fetch_data("Users")
            driver_row = df_users[(df_users["First Name"] == st.session_state.user_name) & (df_users["Category"] == "Driver")].iloc[0]
            
            current_brand = driver_row["Vehicle Brand"] if pd.notna(driver_row["Vehicle Brand"]) else ""
            current_type = driver_row["Vehicle Type"] if pd.notna(driver_row["Vehicle Type"]) else "Voiture"
...
            new_type = st.selectbox("Type de Véhicule", ["Voiture", "Moto", "Camionnette"], index=["Voiture", "Moto", "Camionnette"].index(current_type) if current_type in ["Voiture", "Moto", "Camionnette"] else 0, key="driver_type")
...
            # Quitte pour éviter d'afficher le reste de la page si profil incomplet
        return
        
    df_trips = fetch_data("Trips")
    
