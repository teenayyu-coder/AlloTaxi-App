...
# =======================================================
SHEET_SCHEMAS = {
    "Users": ["Category", "First Name", "Phone", "Password",
...
INITIAL_DATA = {
    "Users": [
        {"Category": "Admin", "First Name": "admin", "Phone": "000000000",
         "Password": "8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918",
         "Vehicle Brand": "", "Vehicle Type": "", "Engine Displacement": "", "Is Online": False},
        {"Category": "Client", "First Name": "testclient", "Phone": "032111111",
         "Password": "5e884898da28047151d0e56f8dc6292773603d0d6aabf35d2153c3e017d23d8c",
         "Vehicle Brand": "", "Vehicle Type": "", "Engine Displacement": "", "Is Online": False},
        {"Category": "Driver", "First Name": "testdriver", "Phone": "034222222",
         "Password": "5e884898da28047151d0e56f8dc6292773603d0d6aabf35d2153c3e017d23d8c",
         "Vehicle Brand": "Peugeot", "Vehicle Type": "Voiture", "Engine Displacement": "1.0L", "Is Online": False}
    ],
    "Trips": [
        {"Client Name": "Admin", "Client Phone": "000000000", "Start Point": "Place de l'Indépendance",
         "End Point": "Analakely", "Budget": "5000", "Status": "Available", "Driver": ""},
        {"Client Name": "testclient", "Client Phone": "032111111", "Start Point": "Antananarivo",
         "End Point": "Imerina", "Budget": "15000", "Status": "Accepted", "Driver": "testdriver"}
    ]
}
...
def save_data(data, initial_create=False):
    ...
    if not github_credentials_available():
        st.info("Sauvegarde locale uniquement.")
        return
    api_url = get_github_api_url()
...
def append_row(sheet_name, new_row_dict):
    data = load_data()
    if sheet_name not in data: data[sheet_name] = []
...
def check_password_strength(password):
    if len(password) < 6: return False, "Mot de passe trop court."
...
def show_login_page():
    ...
    if submitted:
        ...
        if row.empty: st.error("Prénom introuvable."); return
        if hash_password(login_pass) != row["Password"].iloc[0]:
            st.error("Mot de passe incorrect."); return
        ...
        st.rerun()
    if st.button("Créer un compte"): st.session_state.page = "register"
...
def show_register_page():
    ...
    if submitted:
        if first_name.lower() == "admin": st.error("Prénom 'admin' réservé."); return
        ok, msg = check_password_strength(password)
        if not ok: st.error(msg); return
        df = fetch_data("Users")
        if first_name in df["First Name"].values: st.error("Ce prénom existe déjà."); return
        new_user = {
            "Category": category,
        ...
        st.session_state.page = "login"
        st.rerun()
    if st.button("Retour"): st.session_state.page = "login"
...
def show_admin_page():
    ...
    if driver_df.empty: st.info("Aucun driver enregistré."); return
    ...
    for _, driver in driver_df.iterrows():
        ...
        driver_list.append({"Driver": driver_name, "Statut Connexion": status_online,
                            "Statut Course": status_race, "Course en Cours": trip_info})
...
def show_client_page():
    ...
    if submitted:
        if not start_point or not end_point: st.error("Veuillez remplir tous les champs."); return
        new_trip = {"Client Name": st.session_state.user_name,
    ...
    if client_trips.empty: st.info("Vous n'avez créé aucune course."); return
    for _, row in client_trips.sort_index(ascending=False).iterrows():
        status_color = {"Available":"status-available","Accepted":"status-accepted",
                        "Completed":"status-completed","Cancelled":"status-cancelled"}.get(row['Status'],"status-completed")
        st.markdown(f"""
            <div class="trip-card">
            ...
            if st.button("❌ Annuler la course", use_container_width=True):
                if update_row_field("Trips", df_index, "Status", "Available") and \
                   update_row_field("Trips", df_index, "Driver", ""):
                    st.warning("Course annulée et remise en attente.")
                    st.rerun()
        return
    avail = df[df["Status"]=="Available"]
    st.header(f"Courses disponibles ({len(avail)})")
    st.markdown("---")
    if avail.empty: st.info("Aucune course disponible."); return
    for df_index, row in avail.iterrows():
        with st.container():
            st.markdown(f"""
                <div class="trip-card">
                    <h3>🚗 Course #{df_index+1}</h3>
                    <p>📍 Départ : {row['Start Point']}</p>
                    <p>📍 Arrivée : {row['End Point']}</p>
                    <p>💰 Budget : {row['Budget']} Ar</p>
                    <p>👤 Client : {row['Client Name']}</p>
                </div>
            """, unsafe_allow_html=True)
            if st.button(f"✅ Accepter cette course #{df_index}"):
                update_row_field("Trips", df_index, "Driver", st.session_state.user_name)
                update_row_field("Trips", df_index, "Status", "Accepted")
                update_user_online_status(st.session_state.user_name, True)
