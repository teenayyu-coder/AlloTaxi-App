...
def github_url():
    repo = st.secrets["GITHUB_REPO"] if github_ok() else DEFAULT_REPO
    file = st.secrets["GITHUB_FILE"] if github_ok() else DEFAULT_FILE
...
def update(sheet, idx, field, val):
    data = load_data()
    data[sheet][idx][field] = val
    save_data(data)

def notify(target, msg, idx):
    data = load_data()
...
def login():
    ...
    if st.button("Se connecter"):
        if name == ADMIN_USERNAME and hashlib.sha256(pwd.encode()).hexdigest() == ADMIN_PASSWORD_HASH:
            st.session_state.update({"ok":True,"role":"Admin","name":name})
            st.rerun()
...
def admin():
    st.title("Admin")
    for i,n in unread("Admin").iterrows():
        if st.button(n["Message"], key=f"a{i}"):
            update("Notifications", i, "Read", True)
            st.rerun()
    st.dataframe(fetch("Users"), use_container_width=True)
    st.dataframe(fetch("Trips"), use_container_width=True)
    ...
    for i,r in df[df["Client Phone"]==st.session_state.phone].iterrows():
        ...
        if r["Status"] in ["Available","Accepted"]:
            if st.button("Annuler", key=f"c{i}"):
                update("Trips", i, "Status", "Cancelled")
                update("Trips", i, "Driver", "")
                notify("Admin","Course annulée",i)
                st.rerun()
...
def driver():
    st.title("Driver")
    for i,n in unread(st.session_state.name).iterrows():
    ...
    if not mine.empty:
        ...
        if st.button("Terminer"):
            update("Trips", idx, "Status", "Completed")
            st.rerun()
        if st.button("Annuler"):
            update("Trips", idx, "Status", "Available")
            update("Trips", idx, "Driver", "")
            notify("Admin","Driver a annulé",idx)
            st.rerun()
        return
    ...
    for i,r in df[df["Status"]=="Available"].iterrows():
        ...
        if st.button("Accepter", key=f"a{i}"):
            update("Trips", i, "Status", "Accepted")
            update("Trips", i, "Driver", st.session_state.name)
            st.rerun()
